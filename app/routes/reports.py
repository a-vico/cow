...
@router.get("/weights")
async def weights_report(date: str | None = None, db: Session = Depends(get_db)):
    """Return weights report as CSV for given date (YYYY-MM-DD). Defaults to today."""
    # parse date param
    report_date = date
    if report_date:
        try:
...
ORDER BY c.name;
    ...
    output.seek(0)
    headers = {
        "Content-Disposition": f"attachment; filename=report_{rd.strftime('%Y%m%d')}.csv"
    }
...
async def milk_report(
    ...
    """Return milk production report as CSV between start_date and end_date (YYYY-MM-DD)."""
...
ORDER BY 1 desc, 2 desc
    ...
    output.seek(0)
    headers = {
        "Content-Disposition": f"attachment; filename=milk_{sd.strftime('%Y%m%d')}_{ed.strftime('%Y%m%d')}.csv"
    }
    return StreamingResponse(
        io.StringIO(output.getvalue()), media_type="text/csv", headers=headers
    )
